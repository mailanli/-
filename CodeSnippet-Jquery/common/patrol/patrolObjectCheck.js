...
$(document).ready(function() {
  ...
  // createAreaList({'area':'area','project':'projList'})
  // computerRoomTable()
  ...
  if (seachParams && (pageFlag == 'edit')) {
    ...
    getAreaProject(['#area', '#projList'], data)
...
$(".container").empty();
var page = $("<div class='mypage' id='patrolObjectDetail'></div>");
var path = '/integration-admin/common/patrol/patrolObjectDetails.html'
$(".container").append(page);
// 加载目标网页
window.loadPage(page[0], path);
}
...
function deleteItem(id) {
  $.adapter_get('', 'patrolObjects/check/'+id, function (d) {
      if (d.data  ) {
...
function deletePatrolObject(id){
  $.adapter_post({},'patrolObjects/delete/'+window.sessionStorage.getItem('deletePatrol'),function (d) {
      $('#delete').modal('hide');
      layer.msg(d.msg)
...
function exprotFile(){
  $.fileDownLoadExport('patrolObjects/export', true);
}
