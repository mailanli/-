...
$(document).ready(function () {
  ...
  sessionStorage.setItem('objectTypeId', 2) //保存巡逻对象类型
...
function add(params) {
  ...
  var path = '/integration-admin/common/patrol/green/patrolObjectAdd.html'
...
function deleteItem(id) {
  $.adapter_get('', 'greenPatrolObjects/check/' + id, function (d) {
    if (d.data) {
...
function findDetail(data) {
  ...
  $(".container").empty();
  var page = $("<div class='mypage' id='patrolObjectDetail'></div>");
  var path = '/integration-admin/common/patrol/green/patrolObjectDetails.html'
  $(".container").append(page);
  // 加载目标网页
  window.loadPage(page[0], path);
}
function deletePatrolObject(id) {
  $.adapter_post({}, 'greenPatrolObjects/delete/' + window.sessionStorage.getItem('deletePatrol'), function (d) {
    $('#delete').modal('hide');
    layer.msg(d.msg)
...
function exprotFile() {
  $.fileDownLoadExport('greenPatrolObjects/export', true);
}
