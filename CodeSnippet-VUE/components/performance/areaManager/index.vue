...
export default {
  ...
  methods: {
    getParams() {
      ...
      _this.isShowGoBack = data ? true : false
      if (_this.$route.path == '/areaManager') {
        _this.regionCity = _this.$route.query.region
        data ? (_this.key = data) : ''
        // 获取数据接口
        // _this.queryAreaData([_this.key])
        // 获取区域总详情数据 - 11.16
        _this.getAreaInfo(_this.key);
        // _this.getAreaDatas(_this.key);
        _this.getAreaCharts(_this.key);
        // 重新触发月份选项
        _this.monthValue = parseInt(_this.$route.query.month) - 1 || 0;
        _this.getAreaDatas(_this.key, _this.monthValue + 1);
      }
    },
    // 条线图
    lineChart(listData) {
    ...
    processTableData(listData, month) {
      ...
      // console.log(obj)
      _this.quotaOptionsFirst = obj.quotaOptionsFirst
      _this.quotaOptions = obj.quotaOptions

      // 筛选数据
      listData = JSON.parse(JSON.stringify(listData))
      listData = _this.FilterHander(listData.detailInfo, month)
      // console.log(listData.length)
      // console.log("_this.quotaOptions",JSON.stringify(_this.quotaOptions))
      for (let i = 0; i < listData.length; i++) {
        let _item = {
          one: '',
          two: '',
    ...
    Filter_ScoreData(FsJson, month) {
      ...
      let i = FsList.length
      while (i--) {
        let _name = FsList[i].elementInfo.name;
        SiList = FsList[i].scoreDetailInfo || []
        let FsData = parseFloat(SiList[month].targetInfo[Fskey])
        if (!FsData) {
          FsList.splice(i, 1)
        } else {
          FsScore = parseFloat(SiList[month].scoreInfo['complete']) //完成值
          FsSelector === 'lt'
