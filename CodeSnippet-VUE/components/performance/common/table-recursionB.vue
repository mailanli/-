<template >
  <el-table-column align="center" :label="itemData.name" min-width="150">
    <template v-for="(stData,stIndex) in itemData.children">
      ...
      <recursion-a v-else :itemData="stData" :itemUpdata="itemUpdata" @dataToTable="dealTable"></recursion-a>
    </template>
  </el-table-column>
</template>

<script>
// import recursionA from './table-recursionA';//
//
export default {
  data() {
    return {
  ...
  beforeCreate: function() {
    this.$options.components.RecursionA = require('./table-recursionA.vue')
  },
  watch: {
    inspectionNum: function(newVal) {
      console.log(newVal)
    }
  },
  created() {},
