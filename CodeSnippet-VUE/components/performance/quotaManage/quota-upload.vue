      ...
      <div slot="header" class="clearfix">
        ...
        <div style="" class="quartMonth">
          ...
          <div style="display:inline-block;width:120px;">
            <el-select v-model="PageHeadManage['quartMonth']" placeholder="季度" align="center" aria-placeholder="">
      ...
      <div class="dataUpload-content">
          ...
          <el-form ref="form" :model="PageHeadManage" label-width="50px">
            ...
            <el-form-item label="指标">
              <el-select v-model="PageHeadManage['pageType']" placeholder="请选择指标展示">
                <el-option v-for="item in PageOption['pageTypeList']" :key="item.elementId" :label="item.label" :value="item.elementId">
                </el-option>
              </el-select>
            </el-form-item>
            <!-- 时间控件: 开始 -->
            ...
            <el-form-item label="季度">
              <el-select v-model="PageHeadManage['pageQuarter']" placeholder="请选择季度">
        ...
        <el-tabs type="border-card" @tab-click="tabHandle">
          ...
          <el-tab-pane label="手工录入">
            <quota-entry :PageHeadManage="PageHeadManage" :PageOption="PageOption"  @pageAuthorityHandle="pageAuthorityHandle"></quota-entry>
          </el-tab-pane>
        </el-tabs>
      </div>
    </el-card>
...
import quotaEntry from './quota-entry.vue'
import { apiServer } from '@/config/index'
import { quotaApiManage } from '@/api/performance'
...
export default {
  ...
  activated: function() {
    this.PageHeadManage['quartMonth'] = '季度' //解决切换路由导致数据重新未加载
  },
  methods: {
    async handleFileChange() {
      ...
      let _pageYear = this.PageHeadManage['pageYear'],
        _pageQuarter = this.PageHeadManage['pageQuarter'],
      ...
      //   return false
      // }
      if (!_pageYear || !_pageYear || !_pageQuarter || !_pageCity) {
        this.$message('请选择年份或季度或类型')
        inputDOM.value = ''
        return false
      }
      ...
      if (
        ...
        this.onPostFile(inputDOM, _pageYear, _pageQuarter)
      } else {
        this.$message.error('请上传后辍为xlsx与xls的文件')
        inputDOM.value = ''
      }
    },
    async onPostFile(inputDOM, pageYear, pageQuarter) {
      ...
      postData.append('quarter', pageQuarter) //季度
      postData.append('type', this.uploadFileType) //文件类型
      postData.append('uploadfile', uploadfile) //上传的文件
      postData.append('name', uploadfile.name) //文件名
      await quotaApiManage(
        this.PageHeadManage['pageCity'],
        '表格上传',
        pageType,
        postData,
        response => {
        ...
        progressEvent => {//上传文件进度
          console.log(progressEvent)
          inputDOM.value = ''
        }
      )
    },
    tabHandle(_this) {//是否点击手工录入
      this.PageHeadManage['pageIsManual'] =
        _this.label == '手工录入' ? true : false
    },
    pageAuthorityHandle(dataJson) {//表格权限
    ...
    excelDownHandle() {
      ...
      let url = ''
      let typeNo=this.PageHeadManage['pageCity']=='区域'?'1':'2'
          url =
           DOMIN+
          '/performance/'+pageType+'/templetExcel?typeNo='+typeNo
