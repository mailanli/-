...
import { quotaApiManage, getElementByType } from '@/api/performance'
...
export default {
  ...
  watch: {
    ...
    'PageHeadManage.pageType': function(pageTypeVal, oldVal) {
      let _pageQuarter = this.PageHeadManage['pageQuarter']
      let _pageCity = this.PageHeadManage['pageCity']
      //--------------- 清空表格操作:开始-----getPageStatusAjax----------
      this.PageManage['pageCurr'] = 1
      this.PageManage['pageTotal'] = 0
      this.tabData['tableData'] = []
      // ---------------清空表格操作:结束---------------
      this.initTable(_pageCity, pageTypeVal, _pageQuarter, '') //初始化表格
    ...
    //季度切换
    'PageHeadManage.pageQuarter': function(pageQuarterVal, oldVal) {
      let pageTypeVal = this.PageHeadManage['pageType']
      let pageCity = this.PageHeadManage['pageCity']
      if (pageTypeVal && pageCity) {
        this.initTable(pageCity, pageTypeVal, pageQuarterVal, '') //初始化表格
      }
      this.getPageStatusAjax(
        this.PageStatus['GET_STATUS'],
        this.PageStatus['UNKNOW']
  ...
  methods: {
    // 初始化表格
    initTable(pageCity, pageTypeVal, pageQuarterVal, pageMonth) {
    ...
    async getPageDataAjax() {
      ...
      let { pageCity, pageYear, pageQuarter, pageType, pageIsManual } = {
        ...this.PageHeadManage
      }
      if (!PageAuthority['read']) {
        return false
      }
      if (!pageYear || !pageType || !pageQuarter || !pageIsManual || !pageCity)
        return false
      pageYear = new Date(pageYear).getFullYear()
      this.PageManage['loading'] = true
      await quotaApiManage(
        pageCity,
        '表格查询',
        pageType,
        {
          typeNo:pageCity=='区域'?'1':'2',
          pageSize,
          pageCurr,
          quarter: pageQuarter,
          year: pageYear
        ...
        response => {
          ...
          this.apiTabelData = response['dataList'] || []
          this.PageManage['pageTotal'] = response['total'] || 0
          this.PageManage['pageVacancy'] = response['vacancy'] || 0 //欠缺填写的区域数量
          vacancyAreaList = response['nameList'] || [] //欠缺填写的区域名称列表
          // 返回null
          if(!response){
            return;
          }
          ...
          this.tabData['tableData'] = this.dataToTabel(this.apiTabelData)
    ...
    async getTableSaveAjax(rowNum, data) {
      ...
      await quotaApiManage(
        ...
        data,
        response => {
          // let List = this.dataKeyType('列表', response) ? response['dataList'] : []
          let List=response['dataList']
          this.PageManage['loading'] = false
          if (Array.isArray(List) && List.length > 0) {
            this.PageManage['pageIsChange'] = false
            List = this.dataToTabel(List)
    ...
    async getPageStatusAjax(apiType, lockStatus) {
      ...
      let { pageYear, pageQuarter, pageType, pageCity } = {
        ...this.PageHeadManage
      }
      let { GET_STATUS, UNKNOW, LOCK, UNLOCK } = { ...this.PageStatus }
      if (!pageYear || !pageType || !pageQuarter || !pageCity) return false
      ...
      for (let i = 0; (rowJson = allTableList[i]); i++) {
        if (rowJson.hasOwnProperty('sqzn_sqlr_complete')) {
          if (
            ...
            rowJson['sqzn_sqlr_complete'] !== '是' &&
            rowJson['sqzn_sqlr_complete'] !== '否'
          ) {
        ...
        rowJson['elementId']=this.PageHeadManage['pageType']
        List.push(rowJson)
      }
